...
public class QuadrotorController : MonoBehaviour {
	...
	float[] GrabPose() {
        ...
        if (udp_client.Available > 0) {
            ...
            if (POSE_LENGTH * sizeof(float) + STAMP_LENGTH * sizeof(int) == poseBytes.Length) {
				...
				float[] pose_empty = new float[]{0, 0, 0, 1, 0, 0, 0};
                return pose_empty;
            }
            return pose;
        } else {
	...
	Vector4 Controller(float[] pose, Rigidbody rb){
		...
		Vector3 b1_d = new Vector3 (e_x.x, e_x.y, 0);
	...
	void FixedUpdate () {
		...
		Vector3 currentPosition = GameObject.FindGameObjectWithTag("Quadrotor").transform.position;
		//float[] pose = {currentPosition.x+3, currentPosition.z, currentPosition.y+1};
